...
pub async fn login(session: Session) -> Redirect {
...
pub async fn callback(Query(params): Query<Callback>, session: Session) -> Redirect {
    let zauth_state = match session.get::<String>("state").await.unwrap() {
        None => return dbg!(Redirect::to("/login")),
        Some(v) => v,
    };
